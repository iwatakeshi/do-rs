...
if [ -d "$PATCHES_DIR" ]; then
    ...
    # Copy pagination module (custom helper not generated by OpenAPI Generator)
    PAGINATION_FILE="${WORK_DIR}/src/pagination.rs"
    PAGINATION_PATCH="${PATCHES_DIR}/pagination.rs"
    if [ -f "$PAGINATION_PATCH" ]; then
        echo "Copying pagination helper module..."
        cp "$PAGINATION_PATCH" "$PAGINATION_FILE"
        echo "  ✓ Copied pagination.rs to src/"
    fi
