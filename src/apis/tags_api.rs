...
pub enum TagsCreateError {
    Status400(models::ErrorWithRootCauses),
    Status401(models::Error),
    Status429(models::Error),
    Status500(models::Error),
    DefaultResponse(models::Error),
...
pub async fn tags_assign_resources(configuration: &configuration::Configuration, tag_id: &str, tags_resource: models::TagsResource) -> Result<(), Error<TagsAssignResourcesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_tag_id = tag_id;
    let p_body_tags_resource = tags_resource;

    let uri_str = format!("{}/v2/tags/{tag_id}/resources", configuration.base_path, tag_id=crate::apis::urlencode(p_path_tag_id));
    ...
    req_builder = req_builder.json(&p_body_tags_resource);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<TagsAssignResourcesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To create a tag you can send a POST request to `/v2/tags` with a `name` attribute.
pub async fn tags_create(configuration: &configuration::Configuration, tags: models::Tags) -> Result<models::TagsCreate201Response, Error<TagsCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_tags = tags;

    let uri_str = format!("{}/v2/tags", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TagsCreate201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TagsCreate201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<TagsCreateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn tags_delete(configuration: &configuration::Configuration, tag_id: &str) -> Result<(), Error<TagsDeleteError>> {
    ...
    let uri_str = format!("{}/v2/tags/{tag_id}", configuration.base_path, tag_id=crate::apis::urlencode(p_path_tag_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<TagsDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn tags_get(configuration: &configuration::Configuration, tag_id: &str) -> Result<models::TagsGet200Response, Error<TagsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_tag_id = tag_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TagsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TagsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn tags_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::TagsList200Response, Error<TagsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn tags_unassign_resources(configuration: &configuration::Configuration, tag_id: &str, tags_resource: models::TagsResource) -> Result<(), Error<TagsUnassignResourcesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_tag_id = tag_id;
    let p_body_tags_resource = tags_resource;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<TagsUnassignResourcesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
