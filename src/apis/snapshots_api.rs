...
pub async fn snapshots_delete(configuration: &configuration::Configuration, snapshot_id: &str) -> Result<(), Error<SnapshotsDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_snapshot_id = snapshot_id;

    let uri_str = format!("{}/v2/snapshots/{snapshot_id}", configuration.base_path, snapshot_id=p_path_snapshot_id.to_string());
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<SnapshotsDeleteError> = serde_json::from_str(&content).ok();
...
pub async fn snapshots_get(configuration: &configuration::Configuration, snapshot_id: &str) -> Result<models::SnapshotsGet200Response, Error<SnapshotsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_snapshot_id = snapshot_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SnapshotsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SnapshotsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SnapshotsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn snapshots_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>, resource_type: Option<&str>) -> Result<models::SnapshotsList200Response, Error<SnapshotsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_per_page = per_page;
    let p_query_page = page;
    let p_query_resource_type = resource_type;

    let uri_str = format!("{}/v2/snapshots", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SnapshotsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SnapshotsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SnapshotsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
