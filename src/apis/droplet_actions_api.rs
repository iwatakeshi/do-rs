...
pub async fn droplet_actions_get(configuration: &configuration::Configuration, droplet_id: i32, action_id: i32) -> Result<models::ActionsGet200Response, Error<DropletActionsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_droplet_id = droplet_id;
    let p_path_action_id = action_id;

    let uri_str = format!("{}/v2/droplets/{droplet_id}/actions/{action_id}", configuration.base_path, droplet_id=p_path_droplet_id, action_id=p_path_action_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ActionsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ActionsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DropletActionsGetError> = serde_json::from_str(&content).ok();
...
pub async fn droplet_actions_list(configuration: &configuration::Configuration, droplet_id: i32, per_page: Option<i32>, page: Option<i32>) -> Result<models::ActionsList200Response, Error<DropletActionsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_droplet_id = droplet_id;
    let p_query_per_page = per_page;
    let p_query_page = page;

    let uri_str = format!("{}/v2/droplets/{droplet_id}/actions", configuration.base_path, droplet_id=p_path_droplet_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ActionsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ActionsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DropletActionsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn droplet_actions_post(configuration: &configuration::Configuration, droplet_id: i32, droplet_actions_post_request: Option<models::DropletActionsPostRequest>) -> Result<models::ActionsGet200Response, Error<DropletActionsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_droplet_id = droplet_id;
    let p_body_droplet_actions_post_request = droplet_actions_post_request;
    ...
    req_builder = req_builder.json(&p_body_droplet_actions_post_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DropletActionsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn droplet_actions_post_by_tag(configuration: &configuration::Configuration, tag_name: Option<&str>, droplet_actions_post_by_tag_request: Option<models::DropletActionsPostByTagRequest>) -> Result<models::DropletActionsPostByTag201Response, Error<DropletActionsPostByTagError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_tag_name = tag_name;
    let p_body_droplet_actions_post_by_tag_request = droplet_actions_post_by_tag_request;

    let uri_str = format!("{}/v2/droplets/actions", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = req_builder.json(&p_body_droplet_actions_post_by_tag_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DropletActionsPostByTag201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DropletActionsPostByTag201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DropletActionsPostByTagError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
