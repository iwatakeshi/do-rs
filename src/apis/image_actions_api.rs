...
pub async fn image_actions_get(configuration: &configuration::Configuration, image_id: i32, action_id: i32) -> Result<models::Action, Error<ImageActionsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;
    let p_path_action_id = action_id;

    let uri_str = format!("{}/v2/images/{image_id}/actions/{action_id}", configuration.base_path, image_id=p_path_image_id, action_id=p_path_action_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Action`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Action`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ImageActionsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To retrieve all actions that have been executed on an image, send a GET request to `/v2/images/$IMAGE_ID/actions`.
pub async fn image_actions_list(configuration: &configuration::Configuration, image_id: i32) -> Result<models::ActionsList200Response, Error<ImageActionsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;

    let uri_str = format!("{}/v2/images/{image_id}/actions", configuration.base_path, image_id=p_path_image_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ImageActionsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn image_actions_post(configuration: &configuration::Configuration, image_id: i32, image_actions_post_request: Option<models::ImageActionsPostRequest>) -> Result<models::Action, Error<ImageActionsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;
    let p_body_image_actions_post_request = image_actions_post_request;
    ...
    req_builder = req_builder.json(&p_body_image_actions_post_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ImageActionsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
