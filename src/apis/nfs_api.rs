...
/// struct for typed errors of method [`nfs_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsCreateError {
...
/// struct for typed errors of method [`nfs_delete`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsDeleteError {
    Status401(models::Error),
...
/// struct for typed errors of method [`nfs_delete_snapshot`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsDeleteSnapshotError {
    Status401(models::Error),
    Status404(models::Error),
    Status429(models::Error),
    Status500(models::Error),
...
/// struct for typed errors of method [`nfs_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsGetError {
    Status401(models::Error),
    Status404(models::Error),
    Status429(models::Error),
    Status500(models::Error),
...
/// struct for typed errors of method [`nfs_get_snapshot`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsGetSnapshotError {
    Status401(models::Error),
    Status404(models::Error),
    Status429(models::Error),
    Status500(models::Error),
...
/// struct for typed errors of method [`nfs_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsListError {
    Status401(models::Error),
    Status404(models::Error),
    Status429(models::Error),
    Status500(models::Error),
...
/// struct for typed errors of method [`nfs_list_snapshot`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum NfsListSnapshotError {
    Status401(models::Error),
    Status404(models::Error),
    Status429(models::Error),
    Status500(models::Error),
...
pub async fn nfs_create(configuration: &configuration::Configuration, nfs_request: models::NfsRequest) -> Result<models::NfsCreateResponse, Error<NfsCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_nfs_request = nfs_request;

    let uri_str = format!("{}/v2/nfs", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_nfs_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::NfsCreateResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::NfsCreateResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<NfsCreateError> = serde_json::from_str(&content).ok();
...
pub async fn nfs_delete(configuration: &configuration::Configuration, nfs_id: &str, region: &str) -> Result<(), Error<NfsDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_nfs_id = nfs_id;
    let p_query_region = region;

    let uri_str = format!("{}/v2/nfs/{nfs_id}", configuration.base_path, nfs_id=crate::apis::urlencode(p_path_nfs_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<NfsDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn nfs_delete_snapshot(configuration: &configuration::Configuration, nfs_snapshot_id: &str, region: &str) -> Result<(), Error<NfsDeleteSnapshotError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_nfs_snapshot_id = nfs_snapshot_id;
    let p_query_region = region;

    let uri_str = format!("{}/v2/nfs/snapshots/{nfs_snapshot_id}", configuration.base_path, nfs_snapshot_id=crate::apis::urlencode(p_path_nfs_snapshot_id));
    let mut req_builder = configuration.client.request(reqwest::Method::DELETE, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<NfsDeleteSnapshotError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To get an NFS share, send a GET request to `/v2/nfs/{nfs_id}?region=${region}`.  A successful request will return the NFS share. 
pub async fn nfs_get(configuration: &configuration::Configuration, nfs_id: &str, region: &str) -> Result<models::NfsGetResponse, Error<NfsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_nfs_id = nfs_id;
    let p_query_region = region;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::NfsGetResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::NfsGetResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<NfsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn nfs_get_snapshot(configuration: &configuration::Configuration, nfs_snapshot_id: &str, region: &str) -> Result<models::NfsSnapshotGetResponse, Error<NfsGetSnapshotError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_nfs_snapshot_id = nfs_snapshot_id;
    let p_query_region = region;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::NfsSnapshotGetResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::NfsSnapshotGetResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<NfsGetSnapshotError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn nfs_list(configuration: &configuration::Configuration, region: &str) -> Result<models::NfsListResponse, Error<NfsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_region = region;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::NfsListResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::NfsListResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<NfsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn nfs_list_snapshot(configuration: &configuration::Configuration, region: &str, share_id: Option<&str>) -> Result<models::NfsSnapshotListResponse, Error<NfsListSnapshotError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_region = region;
    let p_query_share_id = share_id;

    let uri_str = format!("{}/v2/nfs/snapshots", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    req_builder = req_builder.query(&[("region", &p_query_region.to_string())]);
    if let Some(ref param_value) = p_query_share_id {
        req_builder = req_builder.query(&[("share_id", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::NfsSnapshotListResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::NfsSnapshotListResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<NfsListSnapshotError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
