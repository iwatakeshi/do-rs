...
pub async fn floating_ips_action_get(configuration: &configuration::Configuration, floating_ip: &str, action_id: i32) -> Result<models::FloatingIpsActionPost201Response, Error<FloatingIpsActionGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_floating_ip = floating_ip;
    let p_path_action_id = action_id;

    let uri_str = format!("{}/v2/floating_ips/{floating_ip}/actions/{action_id}", configuration.base_path, floating_ip=crate::apis::urlencode(p_path_floating_ip), action_id=p_path_action_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FloatingIpsActionPost201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FloatingIpsActionPost201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FloatingIpsActionGetError> = serde_json::from_str(&content).ok();
...
pub async fn floating_ips_action_list(configuration: &configuration::Configuration, floating_ip: &str) -> Result<models::FloatingIpsActionList200Response, Error<FloatingIpsActionListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_floating_ip = floating_ip;

    let uri_str = format!("{}/v2/floating_ips/{floating_ip}/actions", configuration.base_path, floating_ip=crate::apis::urlencode(p_path_floating_ip));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FloatingIpsActionList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FloatingIpsActionList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FloatingIpsActionListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn floating_ips_action_post(configuration: &configuration::Configuration, floating_ip: &str, floating_ips_action_post_request: Option<models::FloatingIpsActionPostRequest>) -> Result<models::FloatingIpsActionPost201Response, Error<FloatingIpsActionPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_floating_ip = floating_ip;
    let p_body_floating_ips_action_post_request = floating_ips_action_post_request;
    ...
    req_builder = req_builder.json(&p_body_floating_ips_action_post_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FloatingIpsActionPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
