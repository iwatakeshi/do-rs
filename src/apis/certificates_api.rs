...
pub async fn certificates_create(configuration: &configuration::Configuration, certificates_create_request: models::CertificatesCreateRequest) -> Result<models::CertificatesCreate201Response, Error<CertificatesCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_certificates_create_request = certificates_create_request;

    let uri_str = format!("{}/v2/certificates", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_certificates_create_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CertificatesCreate201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CertificatesCreate201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CertificatesCreateError> = serde_json::from_str(&content).ok();
...
pub async fn certificates_delete(configuration: &configuration::Configuration, certificate_id: &str) -> Result<(), Error<CertificatesDeleteError>> {
    ...
    let uri_str = format!("{}/v2/certificates/{certificate_id}", configuration.base_path, certificate_id=crate::apis::urlencode(p_path_certificate_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CertificatesDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To show information about an existing certificate, send a GET request to `/v2/certificates/$CERTIFICATE_ID`.
pub async fn certificates_get(configuration: &configuration::Configuration, certificate_id: &str) -> Result<models::CertificatesCreate201Response, Error<CertificatesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_certificate_id = certificate_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CertificatesGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To list all of the certificates available on your account, send a GET request to `/v2/certificates`.
pub async fn certificates_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>, name: Option<&str>) -> Result<models::CertificatesList200Response, Error<CertificatesListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CertificatesList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CertificatesList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CertificatesListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
