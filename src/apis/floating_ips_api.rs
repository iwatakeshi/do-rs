...
pub async fn floating_ips_create(configuration: &configuration::Configuration, floating_ip_create: models::FloatingIpCreate) -> Result<models::FloatingIpsCreate202Response, Error<FloatingIpsCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_floating_ip_create = floating_ip_create;

    let uri_str = format!("{}/v2/floating_ips", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_floating_ip_create);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FloatingIpsCreate202Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FloatingIpsCreate202Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FloatingIpsCreateError> = serde_json::from_str(&content).ok();
...
pub async fn floating_ips_delete(configuration: &configuration::Configuration, floating_ip: &str) -> Result<(), Error<FloatingIpsDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_floating_ip = floating_ip;

    let uri_str = format!("{}/v2/floating_ips/{floating_ip}", configuration.base_path, floating_ip=crate::apis::urlencode(p_path_floating_ip));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<FloatingIpsDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To show information about a floating IP, send a GET request to `/v2/floating_ips/$FLOATING_IP_ADDR`.
pub async fn floating_ips_get(configuration: &configuration::Configuration, floating_ip: &str) -> Result<models::FloatingIpsGet200Response, Error<FloatingIpsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_floating_ip = floating_ip;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FloatingIpsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FloatingIpsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FloatingIpsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To list all of the floating IPs available on your account, send a GET request to `/v2/floating_ips`.
pub async fn floating_ips_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::FloatingIpsList200Response, Error<FloatingIpsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FloatingIpsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FloatingIpsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FloatingIpsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
