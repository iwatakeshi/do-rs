...
pub async fn reserved_ips_actions_get(configuration: &configuration::Configuration, reserved_ip: &str, action_id: i32) -> Result<models::ReservedIpsActionsPost201Response, Error<ReservedIpsActionsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_reserved_ip = reserved_ip;
    let p_path_action_id = action_id;

    let uri_str = format!("{}/v2/reserved_ips/{reserved_ip}/actions/{action_id}", configuration.base_path, reserved_ip=crate::apis::urlencode(p_path_reserved_ip), action_id=p_path_action_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReservedIpsActionsPost201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReservedIpsActionsPost201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ReservedIpsActionsGetError> = serde_json::from_str(&content).ok();
...
pub async fn reserved_ips_actions_list(configuration: &configuration::Configuration, reserved_ip: &str) -> Result<models::ReservedIpsActionsList200Response, Error<ReservedIpsActionsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_reserved_ip = reserved_ip;

    let uri_str = format!("{}/v2/reserved_ips/{reserved_ip}/actions", configuration.base_path, reserved_ip=crate::apis::urlencode(p_path_reserved_ip));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReservedIpsActionsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReservedIpsActionsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ReservedIpsActionsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn reserved_ips_actions_post(configuration: &configuration::Configuration, reserved_ip: &str, reserved_ips_actions_post_request: Option<models::ReservedIpsActionsPostRequest>) -> Result<models::ReservedIpsActionsPost201Response, Error<ReservedIpsActionsPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_reserved_ip = reserved_ip;
    let p_body_reserved_ips_actions_post_request = reserved_ips_actions_post_request;
    ...
    req_builder = req_builder.json(&p_body_reserved_ips_actions_post_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ReservedIpsActionsPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
