...
pub async fn cdn_create_endpoint(configuration: &configuration::Configuration, cdn_endpoint: models::CdnEndpoint) -> Result<models::CdnCreateEndpoint201Response, Error<CdnCreateEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_cdn_endpoint = cdn_endpoint;

    let uri_str = format!("{}/v2/cdn/endpoints", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_cdn_endpoint);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CdnCreateEndpoint201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CdnCreateEndpoint201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CdnCreateEndpointError> = serde_json::from_str(&content).ok();
...
pub async fn cdn_delete_endpoint(configuration: &configuration::Configuration, cdn_id: &str) -> Result<(), Error<CdnDeleteEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cdn_id = cdn_id;

    let uri_str = format!("{}/v2/cdn/endpoints/{cdn_id}", configuration.base_path, cdn_id=crate::apis::urlencode(p_path_cdn_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CdnDeleteEndpointError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To show information about an existing CDN endpoint, send a GET request to `/v2/cdn/endpoints/$ENDPOINT_ID`.
pub async fn cdn_get_endpoint(configuration: &configuration::Configuration, cdn_id: &str) -> Result<models::CdnCreateEndpoint201Response, Error<CdnGetEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cdn_id = cdn_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CdnGetEndpointError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To list all of the CDN endpoints available on your account, send a GET request to `/v2/cdn/endpoints`.
pub async fn cdn_list_endpoints(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::CdnListEndpoints200Response, Error<CdnListEndpointsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CdnListEndpoints200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CdnListEndpoints200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CdnListEndpointsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn cdn_purge_cache(configuration: &configuration::Configuration, cdn_id: &str, purge_cache: models::PurgeCache) -> Result<(), Error<CdnPurgeCacheError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cdn_id = cdn_id;
    let p_body_purge_cache = purge_cache;

    let uri_str = format!("{}/v2/cdn/endpoints/{cdn_id}/cache", configuration.base_path, cdn_id=crate::apis::urlencode(p_path_cdn_id));
    let mut req_builder = configuration.client.request(reqwest::Method::DELETE, &uri_str);
    ...
    req_builder = req_builder.json(&p_body_purge_cache);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CdnPurgeCacheError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn cdn_update_endpoints(configuration: &configuration::Configuration, cdn_id: &str, update_endpoint: models::UpdateEndpoint) -> Result<models::CdnCreateEndpoint201Response, Error<CdnUpdateEndpointsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_cdn_id = cdn_id;
    let p_body_update_endpoint = update_endpoint;
    ...
    req_builder = req_builder.json(&p_body_update_endpoint);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CdnUpdateEndpointsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
