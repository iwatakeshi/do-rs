...
pub async fn ssh_keys_create(configuration: &configuration::Configuration, ssh_keys: models::SshKeys) -> Result<models::SshKeysCreate201Response, Error<SshKeysCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_ssh_keys = ssh_keys;

    let uri_str = format!("{}/v2/account/keys", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_ssh_keys);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SshKeysCreate201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SshKeysCreate201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SshKeysCreateError> = serde_json::from_str(&content).ok();
...
pub async fn ssh_keys_delete(configuration: &configuration::Configuration, ssh_key_identifier: &str) -> Result<(), Error<SshKeysDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ssh_key_identifier = ssh_key_identifier;

    let uri_str = format!("{}/v2/account/keys/{ssh_key_identifier}", configuration.base_path, ssh_key_identifier=p_path_ssh_key_identifier.to_string());
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SshKeysDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn ssh_keys_get(configuration: &configuration::Configuration, ssh_key_identifier: &str) -> Result<models::SshKeysCreate201Response, Error<SshKeysGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ssh_key_identifier = ssh_key_identifier;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SshKeysGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn ssh_keys_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::SshKeysList200Response, Error<SshKeysListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SshKeysList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SshKeysList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SshKeysListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn ssh_keys_update(configuration: &configuration::Configuration, ssh_key_identifier: &str, ssh_keys_update_request: models::SshKeysUpdateRequest) -> Result<models::SshKeysCreate201Response, Error<SshKeysUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_ssh_key_identifier = ssh_key_identifier;
    let p_body_ssh_keys_update_request = ssh_keys_update_request;
    ...
    req_builder = req_builder.json(&p_body_ssh_keys_update_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SshKeysUpdateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
