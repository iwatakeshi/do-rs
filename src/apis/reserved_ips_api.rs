...
pub async fn reserved_ips_create(configuration: &configuration::Configuration, reserved_ip_create: models::ReservedIpCreate) -> Result<models::ReservedIpsCreate202Response, Error<ReservedIpsCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_reserved_ip_create = reserved_ip_create;

    let uri_str = format!("{}/v2/reserved_ips", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_reserved_ip_create);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReservedIpsCreate202Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReservedIpsCreate202Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ReservedIpsCreateError> = serde_json::from_str(&content).ok();
...
pub async fn reserved_ips_delete(configuration: &configuration::Configuration, reserved_ip: &str) -> Result<(), Error<ReservedIpsDeleteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_reserved_ip = reserved_ip;

    let uri_str = format!("{}/v2/reserved_ips/{reserved_ip}", configuration.base_path, reserved_ip=crate::apis::urlencode(p_path_reserved_ip));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ReservedIpsDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To show information about a reserved IP, send a GET request to `/v2/reserved_ips/$RESERVED_IP_ADDR`.
pub async fn reserved_ips_get(configuration: &configuration::Configuration, reserved_ip: &str) -> Result<models::ReservedIpsGet200Response, Error<ReservedIpsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_reserved_ip = reserved_ip;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReservedIpsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReservedIpsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ReservedIpsGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To list all of the reserved IPs available on your account, send a GET request to `/v2/reserved_ips`.
pub async fn reserved_ips_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::ReservedIpsList200Response, Error<ReservedIpsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ReservedIpsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ReservedIpsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ReservedIpsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
