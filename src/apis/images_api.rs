...
pub async fn images_create_custom(configuration: &configuration::Configuration, image_new_custom: models::ImageNewCustom) -> Result<models::ImagesCreateCustom202Response, Error<ImagesCreateCustomError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_image_new_custom = image_new_custom;

    let uri_str = format!("{}/v2/images", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_image_new_custom);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ImagesCreateCustom202Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ImagesCreateCustom202Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ImagesCreateCustomError> = serde_json::from_str(&content).ok();
...
pub async fn images_delete(configuration: &configuration::Configuration, image_id: i32) -> Result<(), Error<ImagesDeleteError>> {
    ...
    let uri_str = format!("{}/v2/images/{image_id}", configuration.base_path, image_id=p_path_image_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ImagesDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To retrieve information about an image, send a `GET` request to `/v2/images/$IDENTIFIER`. 
pub async fn images_get(configuration: &configuration::Configuration, image_id: &str) -> Result<models::ImagesGet200Response, Error<ImagesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;

    let uri_str = format!("{}/v2/images/{image_id}", configuration.base_path, image_id=p_path_image_id.to_string());
...
pub async fn images_list(configuration: &configuration::Configuration, r#type: Option<&str>, private: Option<bool>, tag_name: Option<&str>, per_page: Option<i32>, page: Option<i32>) -> Result<models::ImagesList200Response, Error<ImagesListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ImagesList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ImagesList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ImagesListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn images_update(configuration: &configuration::Configuration, image_id: i32, image_update: models::ImageUpdate) -> Result<models::ImagesGet200Response, Error<ImagesUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_image_id = image_id;
    let p_body_image_update = image_update;
    ...
    req_builder = req_builder.json(&p_body_image_update);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ImagesUpdateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
