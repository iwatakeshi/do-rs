...
/// struct for typed errors of method [`one_clicks_install_kubernetes`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OneClicksInstallKubernetesError {
...
/// struct for typed errors of method [`one_clicks_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OneClicksListError {
    Status401(models::Error),
    Status429(models::Error),
    Status500(models::Error),
    DefaultResponse(models::Error),
...
pub async fn one_clicks_install_kubernetes(configuration: &configuration::Configuration, one_clicks_create: models::OneClicksCreate) -> Result<models::OneClicksInstallKubernetes200Response, Error<OneClicksInstallKubernetesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_one_clicks_create = one_clicks_create;

    let uri_str = format!("{}/v2/1-clicks/kubernetes", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_one_clicks_create);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::OneClicksInstallKubernetes200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::OneClicksInstallKubernetes200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<OneClicksInstallKubernetesError> = serde_json::from_str(&content).ok();
...
pub async fn one_clicks_list(configuration: &configuration::Configuration, r#type: Option<&str>) -> Result<models::OneClicksList200Response, Error<OneClicksListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_type = r#type;

    let uri_str = format!("{}/v2/1-clicks", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::OneClicksList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::OneClicksList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<OneClicksListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
