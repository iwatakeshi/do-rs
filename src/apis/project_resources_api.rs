...
pub async fn projects_assign_resources(configuration: &configuration::Configuration, project_id: &str, project_assignment: models::ProjectAssignment) -> Result<models::ProjectsAssignResources200Response, Error<ProjectsAssignResourcesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_project_id = project_id;
    let p_body_project_assignment = project_assignment;

    let uri_str = format!("{}/v2/projects/{project_id}/resources", configuration.base_path, project_id=crate::apis::urlencode(p_path_project_id));
    ...
    req_builder = req_builder.json(&p_body_project_assignment);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectsAssignResources200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectsAssignResources200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ProjectsAssignResourcesError> = serde_json::from_str(&content).ok();
...
pub async fn projects_assign_resources_default(configuration: &configuration::Configuration, project_assignment: models::ProjectAssignment) -> Result<models::ProjectsAssignResources200Response, Error<ProjectsAssignResourcesDefaultError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_project_assignment = project_assignment;

    let uri_str = format!("{}/v2/projects/default/resources", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ProjectsAssignResourcesDefaultError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn projects_list_resources(configuration: &configuration::Configuration, project_id: &str, per_page: Option<i32>, page: Option<i32>) -> Result<models::ProjectsListResources200Response, Error<ProjectsListResourcesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_project_id = project_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectsListResources200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectsListResources200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ProjectsListResourcesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn projects_list_resources_default(configuration: &configuration::Configuration, ) -> Result<models::ProjectsListResources200Response, Error<ProjectsListResourcesDefaultError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ProjectsListResourcesDefaultError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
