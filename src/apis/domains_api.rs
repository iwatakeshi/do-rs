...
pub async fn domains_create(configuration: &configuration::Configuration, domain: Option<models::Domain>) -> Result<models::DomainsCreate201Response, Error<DomainsCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_domain = domain;

    let uri_str = format!("{}/v2/domains", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DomainsCreate201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DomainsCreate201Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DomainsCreateError> = serde_json::from_str(&content).ok();
...
pub async fn domains_delete(configuration: &configuration::Configuration, domain_name: &str) -> Result<(), Error<DomainsDeleteError>> {
    ...
    let uri_str = format!("{}/v2/domains/{domain_name}", configuration.base_path, domain_name=crate::apis::urlencode(p_path_domain_name));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DomainsDeleteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// To get details about a specific domain, send a GET request to `/v2/domains/$DOMAIN_NAME`.
pub async fn domains_get(configuration: &configuration::Configuration, domain_name: &str) -> Result<models::DomainsGet200Response, Error<DomainsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_domain_name = domain_name;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DomainsGet200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DomainsGet200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn domains_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::DomainsList200Response, Error<DomainsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DomainsList200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DomainsList200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<DomainsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
