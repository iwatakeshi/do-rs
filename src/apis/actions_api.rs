...
/// struct for typed errors of method [`actions_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ActionsGetError {
...
/// struct for typed errors of method [`actions_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ActionsListError {
    Status401(models::Error),
    Status429(models::Error),
    Status500(models::Error),
    DefaultResponse(models::Error),
...
pub async fn actions_get(configuration: &configuration::Configuration, action_id: i32) -> Result<models::ActionsGet200Response, Error<ActionsGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_action_id = action_id;

    let uri_str = format!("{}/v2/actions/{action_id}", configuration.base_path, action_id=p_path_action_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ActionsGetError> = serde_json::from_str(&content).ok();
...
pub async fn actions_list(configuration: &configuration::Configuration, per_page: Option<i32>, page: Option<i32>) -> Result<models::ActionsList200Response, Error<ActionsListError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_per_page = per_page;
    let p_query_page = page;

    let uri_str = format!("{}/v2/actions", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ActionsListError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
